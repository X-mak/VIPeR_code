...
class KDLoss(LifelongLoss):
    def __init__(self, args, writer=None, viz=None, viz_start=float('inf'), viz_freq=200, counter=None, lamb=100, relational=True, last_only=True, probabilistic=False):
        self._model_t_states: List[nn.Module.T_destination] = []
        self.last_only = last_only
        self.relational = relational
        self.p = probabilistic
        name = 'RKD' if relational else 'KD'
        if probabilistic: name = 'PKD'
        elif relational: name = 'RKD'
        else: name = 'KD'
    ...
    def kl_distillation(self, projectors_memories_frozen, projectors_memories):
        previous_similarity = torch.einsum('nc,cm->nm', [projectors_memories_frozen, projectors_memories_frozen.T])
        d = torch.tensor(projectors_memories.shape[1], dtype=torch.float)
        t = 0.01
        logits_mask = torch.scatter(torch.ones_like(previous_similarity), 1,
                                    torch.arange(previous_similarity.size(0)).view(-1, 1).cuda(), 0)
        q = torch.softmax(previous_similarity * logits_mask / torch.sqrt(d).round()*t, dim=1)
        current_similarity = torch.einsum('nc,cm->nm', [projectors_memories, projectors_memories.T])
        log_p = torch.log_softmax(current_similarity * logits_mask / torch.sqrt(d).round()*t, dim=1)
        loss_distill = torch.nn.functional.kl_div(log_p, q, reduction="batchmean")
        return loss_distill
    ...
    def calc_loss(self, gd: torch.Tensor, img: torch.Tensor) -> torch.Tensor:
        ...
        for model_t in self.model_t:
            ...
            if self.relational:
                ...
                loss += 1.0*self.kl_distillation(gd_t, gd_s) / len(self.model_t)
...
def get_ll_loss(args, writer=None, viz=None, viz_start=float('inf'), viz_freq=200, counter=None):
    ...
    for method, strength in zip(args.ll_method, args.ll_strength):
        ...
        if method == 'mas':
            ...
            extra_kwargs['relational'] = False
            extra_kwargs['probabilistic'] = True
            loss_class = KDLoss
        elif method == 'ifgir':
            loss_class = KDLoss
